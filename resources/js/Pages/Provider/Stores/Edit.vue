...
const form = useForm({
    ...
    lokasi: props.store.lokasi,
    deskripsi: props.store.deskripsi,
    service_fee: props.store.service_fee,
    status: props.store.status,
});

function submit() {
    form.put(route('provider.stores.update', props.store.id));
}
</script>
...
<template>
    ...
    <ProviderLayout>
        <template #header>
            <h1 class="text-xl font-semibold text-gray-800 dark:text-gray-100">Edit Toko — {{ store.nama_toko }}</h1>
        </template>
